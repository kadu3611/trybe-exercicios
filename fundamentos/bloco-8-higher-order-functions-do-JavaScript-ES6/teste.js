...
const preco = (listaProducts, listaPrices) => listaProducts.map((array, index) =>{
    return {[array]: listaPrices[index]}
})
const listaCompleta = preco(products, prices)
...
const allNameStudents = estudantes.filter((primeiroNome) => (primeiroNome.turno === 'Manhã')).map((primeiroNome) =>
`${primeiroNome.nome} ${primeiroNome.sobrenome}`);
